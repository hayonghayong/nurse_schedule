<template>
  <v-container fluid class="pt-4">
    <v-dialog v-model="isDialogOpen.successDialog" width="350" persistent>
      <v-card>
        ...
        <v-card-text>
          ...
          <p class="successMessage">生年月日：{{ patient.birthday }}</p>
          <p class="successMessage">入院日：{{ patient.hospitalization_date }}</p>
          <p class="errorMessage">手術日：{{ patient.surgery_date }}</p>
          <p class="errorMessage">特記事項：{{ patient.memo }}</p>
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="#62ABF8" text @click="closeDialog('successDialog')">登録を続ける</v-btn>
          <v-btn text @click="closeDialog('successDialogList')">患者一覧に戻る</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-card-text class="text-center">
      <v-form ref="form" v-model="valid" lazy-validation>
        <v-row>
          ...
          <v-col cols="12" class="py-0">
            <v-select v-model="patient.sex" :items="gender" label="性別" color="#62ABF8"></v-select>
          </v-col>
          <v-col cols="12" class="py-0">
            <v-text-field
          ...
          <v-col cols="12" class="py-0">
            ...
            >
              <template v-slot:append-outer>
                <date-picker v-model="patient.hospitalization" />
              </template>
            </v-text-field>
          </v-col>
          <v-col cols="12" class="py-0">
